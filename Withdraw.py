...
class withdraw_class:
    def __init__(self,id):
        ...
        self.root.title("Withdraw Window")
        ...
        self.l2=Label(self.f1,text="Withdrawal Amount",background="light blue")
        self.t2=Entry(self.f1,width=30)
        self.b2=Button(self.f1,text='Withdraw',command=self.withdraw)
        ...
        if(self.t2.get()!=""):
            ...
            if r>0:
                if row[0]>(int(self.t2.get())+500):
                    ...
                    DB.DBB().insert(qry)
                    t=DB.DBB().transid()
                    qry='insert into tbtransaction values(%s,%s,"withdraw",%s,"%s","%s")'%(t,self.t1.get(),self.t2.get(),strftime("%d-%m-%Y", gmtime()),strftime("%H:%M:%S", gmtime()))
                    #print(qry)
                    DB.DBB().insert(qry)
